...
object Common {
  ...
  /**
    ...
    * When running datura in --master local, you can put plugins related files in $project_dir/plugins,
    * then these files will be automatically copied to $project_dir/datura-core/target and token in effect if you start datura in IDE tools such as IDEA.
    * When running datura in --master yarn or --master mesos, you can put plugins related files in plugins dir.
    * */
  def appRootDir: Path = {
